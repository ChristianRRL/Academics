...
public:
   ...
   void directionHelper(std::stack<int>&);
...
private:
	...
	void depthFirstSearchAux(int, int, std::vector<std::string>&, std::vector<int>&,
...
void Maze<T>::output(int location) {
	std::cout << myAdjacencyLists[location].data << " ";
}

template <class T>
void Maze<T>::outputAdjacency(int location) {
	for (std::vector<int>::iterator it = myAdjacencyLists[location].adjacencyLists.begin(); 
...
void Maze<T>::breadthFirstSearch()
{
	...
	while (!container.empty())
	{
		...
		for (std::vector<int>::iterator it = myAdjacencyLists[temp].adjacencyLists.begin();
	...
	std::cout << std::endl;
	std::cout << "BreadthFirst Search: " << std::endl;
	directionHelper(content);

}

template <class T>
void Maze<T>::directionHelper(std::stack<int>& content)
{
...
template <class T>
void Maze<T>::depthFirstSearchAux(int current, int time, std::vector<std::string>& status, std::vector<int>& parent,
	std::vector<int>& distance, std::vector<int>& f)
{
	status[current] = "discovered";
	time = time + 1;
	distance[current] = time;

	for (std::vector<int>::iterator it = myAdjacencyLists[current].adjacencyLists.begin();
...
void Maze<T>::dijkstrasAlgorithm() {
	...
	std::vector<int> distLabel(vertices, -1);	// distance labels for vertices
	std::vector<int> predLabel(vertices);		// predecessor labels for vertices
	...
	std::stack<int> content;			// stack to contain results
	...
	while (distLabel[destination] < 0 && !vertexQueue.empty()) {
		...
		for (std::vector<int>::iterator it = myAdjacencyLists[vertex].adjacencyLists.begin();
		...
		std::vector<int> path(distance + 1);		// path of vector type
	...
	// loop if not empty
	while (!content.empty()) {
		std::cout << content.top() << " ";	// output stack
		content.pop();	// pop
	}
}

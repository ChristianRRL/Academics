...
MagicSquare MagicSquare::rotate()
{
   ...
   temp.generateMagicArray();

   for (j = size; j >= 1; j--)
   {
      for (i = 1; i <= size; i++)
      {
         temp.matrix[i][j] = temp.array[k];
         matrix[i][j] = temp.array[k];
         k++;
      }
   }

   //cout << "temp magic square:\n";
   //cout << temp;
   //system("pause");
   ...
   //tempMagicSquare.generateMagicArray();
   //for (j = size; j >= 1; j--)
   //{
   //   for (i = 1; i <= size; i++)
   //   {
   //      tempMagicSquare.matrix[i][j] = array[k];
   //      k++;
   //   }
   //}

   //cout << tempMagicSquare;

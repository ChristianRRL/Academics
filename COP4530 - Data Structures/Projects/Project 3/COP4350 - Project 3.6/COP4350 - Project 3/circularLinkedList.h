...
private:
   ...
   Precondition: None.
   Postcondition: Returns a random value of type int.
   Time Complexity: O(n)
   ----------------------------------------------------------------*/
   T randomNode(T) const;
   /*----------------------------------------------------------------
   Purpose: Generates a random Node.
   Precondition: None.
   Postcondition: Returns a random value of type T.
   Time Complexity: O(n)
   ----------------------------------------------------------------*/
...
T List<T>::runJosephusAlgorithm() {
   ...
   // Create a count that is 2 times the size of the original list
   int size = copy.getSize();
   int randInt = copy.randomInt(size);
   cout << randInt << endl;
   int count;
   ...
   while (copy.getSize() > 1) {
      count = randInt % getSize();
      cout << "count: " << count << endl;
      //randNode = copy.randomNode(startNode);
      cout << randInt << endl;
      //cout << randNode << endl;
...
template <class T>
int List<T>::randomInt(int value) const { // pass by ref?
   srand(time(NULL));
   return (rand() % (value * 2));
}

template <class T>
T List<T>::randomNode(T value) const { // pass by ref?
   int size = getSize();

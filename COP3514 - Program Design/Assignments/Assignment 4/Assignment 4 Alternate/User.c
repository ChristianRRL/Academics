...
int verify_user_data(Date _date_of_birth,char _first_name[],char _last_name[],char _username[], char _email[])
{
	...
	if (verify_email(_email) == 0)
		printf("ERROR: Email\n");
	if (verify_date(_date_of_birth) == 1 && verify_name(_first_name) == 1 && verify_name(_last_name) == 1 && verify_username(_username) == 1 && verify_email(_email) == 1)
		return 1;
	else
		return 0;
}
...
User create_user(Date _date_of_birth,double _height,double _weight,char _first_name[],char _last_name[],char _username[],char _email[])
{
	...
	if(verify_user_data(_date_of_birth, _first_name, _last_name, _username, _email) == 1)
		users.valid = 1;
	else
		users.valid = 0;
...
int verify_email(char _email[])
{
	...
	for (i=0; _email[i]!='@'; i++)
	{
		if (_email[i] < '0' || _email[i] > 'z')
		{
			return 0;
		}
		if (_email[i] > '9' && _email[i] < 'A')
		{
			return 0;
		}
		if (_email[i] > 'Z' && _email[i] < 'a')
		{
			return 0;
		}
	}
	
	char address[100] = "mail.usf.edu";
	char temp[100];
	int pos = 0;
	count = 0;
	for (i=0; _email[i]!='\0'; i++)
